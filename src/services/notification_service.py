...
class NotificationService:
    ...
    async def notify_threshold_change(
        self, chat_title: str, old_threshold: float, new_threshold: float
    ) -> None:
        """Notify about significant threshold changes."""
        if abs(
            old_threshold - new_threshold
        ) >= self.thresholds.threshold_change and self._should_notify(
            "threshold_change", chat_title
        ):
            await self._send_notification(
    ...
    async def notify_high_activity(
        self, chat_title: str, msg_count: int, timespan_hours: float
    ) -> None:
        """Notify about unusually high chat activity."""
        # Calculate messages per hour
        msgs_per_hour = msg_count / timespan_hours if timespan_hours > 0 else msg_count
        logger.debug(
            f"High activity check: {msgs_per_hour} msgs/hour >= {self.thresholds.high_activity_count}/{self.thresholds.high_activity_hours}"
        )

        threshold_check = (
            msgs_per_hour
            >= self.thresholds.high_activity_count / self.thresholds.high_activity_hours
        )
        cooldown_check = self._should_notify("high_activity", chat_title)
        logger.debug(
            f"Notification decision: threshold_check={threshold_check}, cooldown_check={cooldown_check}"
        )
    ...
    async def notify_low_response_rate(self, chat_title: str, rate: float) -> None:
        ...
        if rate < self.thresholds.low_response_rate and self._should_notify(
            "low_response_rate", chat_title
        ):
            await self._send_notification(
    ...
    async def notify_error(self, error_type: str, details: str) -> None:
        ...
        if self._should_notify("error", error_type):
            await self._send_notification(f"❌ System Error - {error_type}:\n{details}")
    ...
    async def notify_shutdown(self) -> None:
        ...
        await self._send_notification("🔄 vAIlentin 2.0 bot is shutting down...")

    async def notify_daily_summary(
        self, total_messages: int, response_rate: float, active_chats: int
    ) -> None:
    ...
    def _should_notify(self, notification_type: str, key: str) -> bool:
        ...
        if cache_key in self._last_notification:
            ...
            logger.debug(
                f"Cooldown check for {cache_key}: time_since_last={time_since_last}, cooldown={cooldown}, should_notify={should_notify}"
            )
