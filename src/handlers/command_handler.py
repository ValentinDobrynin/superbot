...
async def help_command(message: Message, session: AsyncSession) -> None:
    ...
    help_text = (
        "🤖 Команды бота\n"
        "Все команды доступны только владельцу в личке с ботом.\n\n"
        "📰 Дайджест\n"
        "/digest — за вчера (00:00–23:59 МСК) — дефолт\n"
        "/digest today — за сегодня (с начала дня до сейчас)\n"
        "/digest yesterday — явно за вчера\n"
        "/digest 2026-05-08 — за конкретную дату (ISO)\n"
        "/today — то же, что /digest today\n"
        "ℹ️ Авто-дайджест уходит каждый день в 23:50 МСК.\n\n"
        "🗂 Глоссарий чатов (классификация)\n"
        "/glossary — список всех чатов, страницы по 10\n"
        "/glossary unset — только ❓ не классифицированные\n"
        "/glossary suggest — LLM сам предложит классификацию\n"
        "                    для всех ❓ с активностью за 7 дней\n"
        "В каждой строке: 💼 Бизнес · 👤 Личный · 🤝 Микс · ⏭ позже\n"
        "Хедер-кнопки: 🪄 Suggest · 🔍 ❓ / 🔄 Все · ⏮ ◀ ▶\n\n"
        "✅ Действия (из дайджеста)\n"
        "/commits — открытые коммиты (✅ Сделано / 🗑 Отменить)\n"
        "/events — предстоящие события (✅ Прошло / 🗑 Отменить)\n"
        "ℹ️ События автоматически становятся «past», когда дедлайн прошёл.\n\n"
        "🤝 Telegram Business\n"
        "/business — статус observer'а (подключения, чаты, пауза)\n"
        "/business on — снять паузу (сохраняем сообщения снова)\n"
        "/business off — поставить на паузу (бот игнорирует Business-сообщения)\n\n"
        "📊 Чаты и аналитика\n"
        "/status — статус бота + статистика по выбранному чату\n"
        "/list_chats — все чаты с их настройками\n"
        "/summ — интерактивная суммаризация чата за период\n\n"
        "⚙️ Настройки конкретного чата\n"
        "/setmode — silent mode (бот читает, но не отвечает)\n"
        "/set_style — стиль ответов (work / friendly / mixed)\n"
        "/set_probability — вероятность ответа на обычное сообщение\n"
        "/set_importance — порог важности (0.0–1.0)\n"
        "/smart_mode — smart mode (LLM-фильтр по важности)\n\n"
        "🎨 Стили и обучение\n"
        "/upload — загрузить дамп переписки для обучения стилю\n"
        "/refresh — пересобрать стиль из истории чата\n"
        "/style — просмотреть/выбрать профили стилей\n"
        "/test — отправить тестовое сообщение в выбранный чат\n\n"
        "🛠 Контент\n"
        "/tag — теги сообщений (add/remove/list/stats); без аргументов — usage\n"
        "/thread — треды (info/list/new/close); без аргументов — usage\n\n"
        "🔒 Система\n"
        "/help — эта справка\n"
        "/shutdown — глобальный silent mode (вкл / выкл)\n\n"
        "🤖 Автоматика\n"
        "• 23:50 МСК — авто-дайджест за прошедший день.\n"
        "• 04:00 МСК — TTL-чистка messages.text старше 30 дней\n"
        "  и пометка прошедших событий как past."
    )
