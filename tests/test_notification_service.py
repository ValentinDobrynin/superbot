...
def notification_service(mock_bot):
    return NotificationService(bot=mock_bot, owner_id=12345, thresholds=NotificationThresholds())
...
async def test_notify_style_change(notification_service):
    ...
    await notification_service.notify_style_change(
        chat_title="Test Chat", old_style="Formal", new_style="Casual"
    )

    notification_service.bot.send_message.assert_called_once_with(
        12345, "🎨 Style changed for Test Chat:\nFrom: Formal\nTo: Casual"
    )
...
async def test_notify_threshold_change(notification_service):
    ...
    await notification_service.notify_threshold_change(
        chat_title="Test Chat", old_threshold=0.3, new_threshold=0.6
    )
...
async def test_notify_high_activity(notification_service):
    ...
    await notification_service.notify_high_activity(
        chat_title="Test Chat", msg_count=120, timespan_hours=1
    )
...
async def test_notify_low_response_rate(notification_service):
    ...
    await notification_service.notify_low_response_rate(chat_title="Test Chat", rate=0.05)

    notification_service.bot.send_message.assert_called_once_with(
        12345, "📉 Low response rate in Test Chat:\n" "Current rate: 5.0%\n" "Threshold: 10.0%"
    )
...
async def test_error_notification(notification_service):
    ...
    await notification_service.notify_error(
        error_type="Database Error", details="Connection failed"
    )

    notification_service.bot.send_message.assert_called_once_with(
        12345, "❌ System Error - Database Error:\nConnection failed"
    )
...
async def test_daily_summary(notification_service):
    ...
    await notification_service.notify_daily_summary(
        total_messages=500, response_rate=0.35, active_chats=5
    )
...
async def test_startup_shutdown(notification_service):
    ...
    notification_service.bot.send_message.assert_called_with(
        12345, "🚀 vAIlentin 2.0 bot started successfully!\n" "Use /help to see available commands."
    )
    ...
    notification_service.bot.send_message.assert_called_with(
        12345, "🔄 vAIlentin 2.0 bot is shutting down..."
    )
...
async def test_high_activity_edge_cases(notification_service):
    ...
    await notification_service.notify_high_activity(
        chat_title="Test Chat", msg_count=100, timespan_hours=1.0
    )
    assert notification_service.bot.send_message.call_count == 1
    ...
    await notification_service.notify_high_activity(
        chat_title="Test Chat", msg_count=99, timespan_hours=1.0
    )
    ...
    await notification_service.notify_high_activity(
        chat_title="Test Chat", msg_count=15, timespan_hours=0.1
    )
    assert (
        notification_service.bot.send_message.call_count == 1
    )  # Should trigger as > 100 msgs/hour
...
async def test_message_formatting_numbers(notification_service):
    ...
    await notification_service.notify_threshold_change(
        chat_title="Test Chat", old_threshold=0.333333, new_threshold=0.666667
    )
...
async def test_long_chat_title(notification_service):
    ...
    await notification_service.notify_style_change(
        chat_title=long_title, old_style="Old", new_style="New"
    )
...
async def test_custom_thresholds(mock_bot, custom_thresholds):
    ...
    service = NotificationService(bot=mock_bot, owner_id=12345, thresholds=custom_thresholds)

    # Test high activity with custom threshold
    await service.notify_high_activity(chat_title="Test Chat", msg_count=150, timespan_hours=2)
    ...
    await service.notify_threshold_change(
        chat_title="Test Chat", old_threshold=0.3, new_threshold=0.5
    )
    assert service.bot.send_message.call_count == 0  # Should not trigger (below 0.3 change)

    await service.notify_threshold_change(
        chat_title="Test Chat", old_threshold=0.3, new_threshold=0.7
    )
