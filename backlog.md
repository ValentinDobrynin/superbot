...
- **Status:** 🆕 To Do
- **Priority:** Medium
- **Component:** `src/database/migrations/env.py`, `requirements.txt`

**Problem Description**

`src/database/migrations/env.py` использует `psycopg2` для синхронных
миграций. Это вторая зависимость на драйвер Postgres (рантайм бота —
`asyncpg`). История с BUG-004 показала, что эту неявную зависимость
легко не заметить и выкосить, что ломает прод-деплой.

**Expected Behavior**

`env.py` использует `async_engine_from_config` / `AsyncEngine` поверх
`asyncpg`. `psycopg2-binary` можно убрать из зависимостей.

**Technical Details**

- Шаблон Alembic уже поддерживает async — см. их `async.py.mako`.
- В `run_migrations_online` использовать
  `create_async_engine(settings.get_async_database_url(), poolclass=NullPool)`
  и `async with engine.connect() as conn: await conn.run_sync(do_run_migrations)`.
- В `run_migrations_offline` — оставить как есть, но без
  `replace("postgresql+asyncpg://", "postgresql://")`, потому что
  offline-режим вообще не открывает соединение.
- После рефакторинга — выкатить отдельным деплоем с проверкой,
  что `alembic upgrade head` работает на проде.
- Только после успешного выкатывания — удалить `psycopg2-binary` из
  зависимостей.

**Acceptance Criteria**

- [ ] `env.py` не импортирует `psycopg2`.
- [ ] `alembic upgrade head` работает на Render (build не падает).
- [ ] `psycopg2-binary` удалён из `requirements.txt` и `pyproject.toml`.
- [ ] Тесты, гоняющие миграции (если есть), зелёные.
